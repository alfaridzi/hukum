@extends('app.app')
@section('page-title', 'Log Registrasi Naskah Masuk')
@push('css')
...
@endif
	...
	<table class="table table-bordered table-responsive" id="table-naskah-masuk">
		...
		<tbody>
			...
			<tr>
				<td>{{ $no++ }}</td>
				<td><a href="{{ url('log/registrasi-naskah-masuk/detail/'.$data->id_naskah) }}">@foreach($data->penerima as $dataPenerima)
					{{ $dataPenerima->get_status_naskah() }}
					@endforeach
				</a></td>
				<td><a href="{{ url('log/registrasi-naskah-masuk/detail/'.$data->id_naskah) }}">{{ $data->urgensi->tingkat }}</a></td>
				...
				<td><a href="{{ url('log/registrasi-naskah-masuk/detail/'.$data->id_naskah) }}">{{ $data->tanggal_naskah }}</a></td>
				<td><a href="{{ url('log/registrasi-naskah-masuk/detail/'.$data->id_naskah) }}">{{ $data->tanggal_registrasi }}</a></td>
				<td>
					@if($data->getPenerima->count() == 1)
					<form action="{{ url('log/registrasi-naskah-masuk/delete/'.$data->id_naskah) }}" method="post">
						@csrf
